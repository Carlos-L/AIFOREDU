...
menu2 = st.sidebar.selectbox("大模型选择", ["DeepSeek","DeepSeek_V3_Ali", "文心一言", "ChatGPT"])
...
if menu == "学员信息分析":
    ...
    if st.button("分析"):
        ...
        user_msg = f"""
                ...
                """
        # st.write(user_msg)
        # result = AI_analysis(system_msg,user_msg)
...
## 学科计划安排 
    ...
    st.header("学习计划安排")
    # user_date = st.date_input("选择目标考试的日期")
    menu_T = st.selectbox('请选择你的计划时间跨度',['当天','周','月','半年'])
    ...
    if menu_T=='当天':
        ...
        zhouqi = f'现在是{today}，请生成今天的学习计划安排，按照小时划分，计划开始时间为{start_time}，结束时间为{end_time}'
    elif menu_T == '周':
